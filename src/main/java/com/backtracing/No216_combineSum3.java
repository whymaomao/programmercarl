...
public class No216_combineSum3 {

    static List<List<Integer>> result = new ArrayList<>();

    public static void backtracing(LinkedList<Integer> backtraceList, int n, int k, int sum, int start) {
        ...
        for(int i = start; i <= 9 ; i ++) {
            sum += i;
            backtraceList.add(i);
            backtracing(backtraceList, n, k, sum,i + 1);
            sum -= i;
    ...
    public static List<List<Integer>> combinationSum3(int k, int n) {
        ...
        int sum = 0;
        backtracing(backtraceList, n, k, sum, 1);
        return result;
    }
